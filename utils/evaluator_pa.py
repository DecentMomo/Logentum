...
def calculate_similarity(template1, template2):
    template1 = normalize_template(template1)
    template2 = normalize_template(template2)
...
def calculate_similarity_accuracy(groundtruth_df, parsedresult_df, filter_templates=None):
    ...
    similarities = []
    for index in groundtruth_df.index:
        similarities.append(
            calculate_similarity(
                groundtruth_df.at[index, 'EventTemplate'],
                parsedresult_df.at[index, 'EventTemplate'],
            )
        )

    sa = float(sum(similarities)) / len(similarities)
    print('Similarity_Accuracy (SA): {:.4f}'.format(sa))
    return sa


def calculate_relaxed_parsing_accuracy(groundtruth_df, parsedresult_df, threshold=0.7, filter_templates=None):
    if filter_templates is not None:
        groundtruth_df = groundtruth_df[groundtruth_df['EventTemplate'].isin(filter_templates)]
        parsedresult_df = parsedresult_df.loc[groundtruth_df.index]
    ...
    rpa = float(correct) / len(groundtruth_df)
    print('Relaxed_Parsing_Accuracy (RPA@{:.2f}): {:.4f}'.format(threshold, rpa))
    return rpa
